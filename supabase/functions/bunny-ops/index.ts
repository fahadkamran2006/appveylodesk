...
const BUNNY_STREAM_STORAGE_ZONE = `vz-${BUNNY_STREAM_LIBRARY_ID}`;
...
// Helper: base64url encoding (not used for signing anymore, but keeping for potential future use)
...
// Proxy download for Bunny Stream videos via Storage API (bypasses Token Auth on Pull Zone)
async function handleDownloadStream(
  ...
  // Check if Storage API key is configured
  if (!BUNNY_STREAM_STORAGE_KEY) {
    console.error("BUNNY_STREAM_STORAGE_KEY not configured");
    return new Response(JSON.stringify({ 
      error: "Storage API not configured",
      details: "Please configure BUNNY_STREAM_STORAGE_KEY secret"
    }), {
      status: 500,
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  ...
  const storageResponse = await fetch(storageUrl, {
    method: "GET",
    headers: {
      "AccessKey": BUNNY_STREAM_STORAGE_KEY,
    },
  });
  
  if (!storageResponse.ok) {
    console.error(`Storage API fetch failed: ${storageResponse.status}`);
    
    // If original file not found, try MP4 fallback via Stream CDN
    if (storageResponse.status === 404) {
      console.log("Original file not found in storage, trying MP4 fallback...");
      
      // Try getting video info to check if MP4 fallback is available
      ...
      if (videoInfoResponse.ok) {
        ...
        console.log(`Video status: ${videoInfo.status}, MP4 Fallback: ${videoInfo.mp4Fallback}`);
        ...
        for (const res of resolutions) {
          ...
          const mp4Response = await fetch(mp4StorageUrl, {
            method: "GET",
            headers: { "AccessKey": BUNNY_STREAM_STORAGE_KEY },
          });
          
          if (mp4Response.ok) {
            console.log(`MP4 fallback ${res}p download succeeded`);
      ...
      return new Response(JSON.stringify({ 
        ...
        details: "The original video file may not be stored. Enable 'Keep Original Files' in Bunny Stream settings.",
      }), {
        status: 404,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    ...
    return new Response(JSON.stringify({ 
      error: `Storage API error: ${storageResponse.status}`,
      details: "Failed to fetch video from Bunny Storage API"
    }), {
  ...
  console.log(`Storage API fetch succeeded, size: ${storageResponse.headers.get("Content-Length")}`);
  
  // Sanitize filename and stream response
  const downloadFilename = sanitizeFilename(fileName || `video-${videoId}`);
