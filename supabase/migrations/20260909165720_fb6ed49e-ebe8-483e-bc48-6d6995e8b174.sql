...
BEGIN
  ...
  _emp_type := COALESCE(NULLIF(inv.metadata ->> 'employment_type', '')::employment_type, 'freelance'::employment_type);
  _salary := NULLIF(inv.metadata ->> 'monthly_salary', '')::numeric;

  INSERT INTO public.profiles (id, email, full_name, agency_id, onboarding_completed, employment_type)
  VALUES (auth.uid(), jwt_email, inv.full_name, inv.agency_id, false, _emp_type)
  ON CONFLICT (id) DO UPDATE
    SET email = EXCLUDED.email,
        agency_id = EXCLUDED.agency_id,
        full_name = COALESCE(public.profiles.full_name, EXCLUDED.full_name),
        employment_type = CASE WHEN inv.role = 'editor'::app_role THEN EXCLUDED.employment_type ELSE public.profiles.employment_type END;
  ...
  IF inv.role = 'editor'::app_role AND _emp_type = 'salaried'::employment_type THEN
    INSERT INTO public.employee_compensation (user_id, monthly_salary)
    VALUES (auth.uid(), _salary)
